 ...
 *   -> that call construct0r()
 *   -> create an attack(uint pwd) function that
 *   -> with a js script you can know the (uint private password) by using "owner.provider.getStorageAt(SimpleTrick.address, 2);"
 *   -> send 0.001 eth to GatekeeperThree
 *   -> make a receive() function that revert()
 *
 */
